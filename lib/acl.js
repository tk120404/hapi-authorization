 ...
 * @returns {*}
 */
exports.checkRoles = function(user, role, validateFeature, feature, hierarchy) {

    if ((!user) || (!internals.isGranted(user.role, validateFeature, user.feature, role, feature, hierarchy))) {
 ...
 */
internals.isGranted = function(userRole, validateFeature, userFeature, requiredRole, requiredFeature, hierarchy) {
    let userRoles = null;

    featureCheck = validateFeature && userFeature == requiredFeature;
